...
fun String.cleanToAnnotatedString(): AnnotatedString {
    ...
    // 1. Initial decode
    val firstPass = HtmlCompat.fromHtml(this, HtmlCompat.FROM_HTML_MODE_LEGACY).toString()
    ...
    val noJS = firstPass.replace(Regex("""document\.getElementById\(.*?\)\.innerHTML\s*=\s*[`'"].*?[`'"]\s*;""", RegexOption.DOT_MATCHES_ALL), "").trim()
